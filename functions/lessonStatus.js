/**
 * Canonical in-person lesson lifecycle status constants and transition validators.
 */
...
const LESSON_STATUS = {
  ...
  IN_PROGRESS: 'in_progress', // synonym for in_session
...
const TRAVEL_LIFECYCLE_STATUSES = [
...
function isActiveLessonStatus(status) {
  if (!status) return false;
  return ACTIVE_LESSON_STATUSES.includes(String(status).toLowerCase());
}

function isTravelLifecycleStatus(status) {
  if (!status) return false;
