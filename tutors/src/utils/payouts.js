import { PLATFORM_FEE_RATE, TUTOR_PAYOUT_RATE } from '../constants/onboarding';
...
export function formatWeekRangeLabel(weekStartInput, weekEndInput) {
  ...
  const startFormatted = `${weekStart.getUTCDate()} ${weekStart.toLocaleString('default', { month: 'short' })}`;
  const endFormatted = `${weekEnd.getUTCDate()} ${weekEnd.toLocaleString('default', { month: 'short' })}`;
...
export function groupSessionsByWeek(sessions = []) {
  ...
  sessions.forEach((session) => {
    ...
    existing.grossAmount += amounts.totalAmount;
    existing.tutorAmount += amounts.tutorAmount;
    existing.platformAmount += amounts.platformAmount;
