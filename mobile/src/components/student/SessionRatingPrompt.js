...
export function SessionRatingPrompt({ session, role = 'student', onHandled }) {
  ...
  const statusCopy = session.status === 'completed' ? 'Session ended' : 'Session canceled';
  const counterpart = role === 'student'
    ? (session.tutorName || session.tutorId || 'your tutor')
    : (session.studentName || session.studentId || 'your student');
