...
function buildFallbackResult(file, fileType = null) {
  return buildExtractionResult({
    file,
    fileType: fileType || detectAttachmentType(file) || 'image',
...
export async function extractSingleAttachment(file) {
  ...
  if (fileType === 'image') {
    ...
    try {
      ...
      return buildFallbackResult(file, fileType);
    }
  }
...
export async function extractAttachments(files = [], onProgress) {
  ...
  for (let index = 0; index < files.length; index += 1) {
    ...
    try {
      ...
      console.debug('[attachmentExtraction] unhandled extraction error', {
        fileName: file?.name,
        error: error?.message,
      });
      result = buildFallbackResult(file);
    }
