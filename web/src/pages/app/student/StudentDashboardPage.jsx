...
export default function StudentDashboardPage() {
  ...
  const onFileChange = (event) => {
    ...
    const existingKeys = new Set(attachments.map((file) => getAttachmentKey(file)));
    const newFilesForExtraction = validFiles.filter((file) => !existingKeys.has(getAttachmentKey(file)));
    const existingFilesToReextract = validFiles.filter((file) => existingKeys.has(getAttachmentKey(file)));

    setAttachments((prev) => [...prev, ...newFilesForExtraction]);

    const filesForExtraction = [...newFilesForExtraction, ...existingFilesToReextract];

    if (filesForExtraction.length) {
      console.debug('[attachmentExtraction] extraction batch started', { fileCount: filesForExtraction.length });
      setAttachmentExtractionStatusByKey((prev) => {
        const next = { ...prev };
        filesForExtraction.forEach((file) => {
          next[getAttachmentKey(file)] = 'extracting';
        });
        return next;
      });

      extractAttachments(filesForExtraction, (result, index) => {
        const file = filesForExtraction[index];
        const fileKey = getAttachmentKey(file);
        console.debug('[attachmentExtraction] extraction state update', {
      ...
      console.debug('[attachmentExtraction] no new files detected for extraction');
    }
  ...
  const removeAttachment = (indexToRemove) => {
    ...
    if (removed) {
      ...
      console.debug('[attachmentExtraction] cleaning up extraction state', { fileName: removed.name });
